...
# Veri setini yükle
veriler = pd.read_csv("EV-USA-Monthly_new1.csv", sep=";")
month_mapping = {'Jan': 1, 'Feb': 2, 'Mar': 3, 'Apr': 4, 'May': 5, 'Jun': 6, 'Jul': 7, 'Aug': 8, 'Sep': 9, 'Oct': 10, 'Nov': 11, 'Dec': 12}

# Ay ve Yıl sütununu birleştirerek yeni bir sütun oluştur
veriler['Month_Year'] = veriler.iloc[:, 0].apply(lambda x: str(month_mapping[x.split('-')[0]]) + '.' + x.split('-')[1])

# Girdi (x) ve Çıktı (y) sütunlarını seç
x = veriler[['Month_Year','GP','DP','EP','MRGDPI','TNL','TREP','PPI(BM)','CPI(NV)','NALR']]
y = veriler['PHEV']

# Virgül yerine nokta kullanarak sayıları düzelt
y = y.replace({',': '.'}, regex=True)
x = x.replace({',': '.'}, regex=True)

# Giriş ve çıkış verilerini eğitim ve test setlerine ayır
X = x.iloc[41:,:]
Y = y.iloc[41:]

from sklearn.model_selection import KFold

# Veri setini 5 kategoriye bölecek şekilde KFold 
kf = KFold(n_splits=10, shuffle=True, random_state=42)

# Model performanslarını saklamak için boş listeler oluştur
mse_cnn_list= []
r2_cnn_list= []
mae_cnn_list= []
rmse_cnn_list= []
...
# Her bir KFold bölünmesi için döngü
